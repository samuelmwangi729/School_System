...
from Institutions.models import Institution
from InstitutionClasses.models import InstitutionClass, InstitutionClass as Classes
from rest_framework_simplejwt.serializers import TokenObtainPairSerializer
User = get_user_model()

class UserSerializer(serializers.ModelSerializer):
    institution_name = serializers.CharField(write_only=True)  # input only
    class_code = serializers.CharField(write_only=True,default="None")  # input only
    ...
    def validate(self, attrs):
        ...
        #get the institution classes also
        try:
            user_class = Classes.objects.get(institution=institution,class_code=class_code)
        except Classes.DoesNotExist:
            raise serializers.ValidationError({
                "class_code":"class does not exist"
                })

        attrs['institution'] = institution  # Add the institution object to the validated data
        attrs['user_class'] = user_class  # Add the class object to the validated data
        return attrs

    def create(self, validated_data):
        validated_data.pop('institution_name')  # No longer needed
        password = validated_data.pop('password')
        password = validated_data.pop('class_code')
...
class UpdateUserSerializer(serializers.ModelSerializer):
    ...
    def validate(self,attrs):
        ...
        try:
            ...
            raise serializers.ValidationError({
                "teacher_name":"teacher does not exist"
                })
        try:
